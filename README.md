...
# Node-RED Flows для Home Assistant

## Описание
Этот репозиторий содержит потоки Node-RED для интеграции с Home Assistant, включая:
- Мониторинг погодных условий
- Оповещения о грозовых разрядах
- Управление поливом и дренажем

## Потоки

### 1. Погодная станция (ws.json)
Основной поток для мониторинга погодных условий. Включает:
- Сбор данных с датчиков
- Расчет индекса комфорта
- Отправку уведомлений в Telegram
- Интеграцию с OpenMeteo для прогнозов

### 2. Оповещение о молнии (lightning_flow.json)
Поток для мониторинга грозовых разрядов:
- Отслеживание ударов молнии через сенсор `sensor.gw2000a_last_lightning_strike`
- Получение расстояния до молнии через `sensor.gw2000a_lightning_strike_distance`
- Фильтрация по расстоянию (оповещения при ударах ближе 10 км)
- Форматирование и отправка уведомлений в Telegram
- Автоматическое обновление при каждом новом ударе молнии

### 3. Управление поливом (watering_flow.json)
Поток для автоматического управления поливом и дренажем:
- Мониторинг влажности почвы и других параметров
- Расчет необходимости полива с учетом:
  ...
  - Скорости ветра
  - Времени суток
- Расчет объема воды для полива
- Рекомендации по дренажу
- Отправка уведомлений в Telegram

## Установка
1. Импортируйте потоки в Node-RED
...
- Node-RED
- Home Assistant
- Интеграция с погодной станцией
- Telegram бот
